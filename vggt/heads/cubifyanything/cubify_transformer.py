...
from cubifyanything.vggt.models.vggt import VGGT
...
class CubifyTransformer(nn.Module):
    ...
    def inference(
        ...
        # 1. 特征提取（主干网络）
        sensor = batched_sensors[self.sensor_name] #wide
        print(sensor['image'])
        features = self.backbone(sensor)
        # 2. 准备多尺度特征
        srcs, masks, pos_embeds = [], [], []
        # print(sensor['image'].data.tensor.shape) #([1, 3, 512, 512])
        ...
        src_flatten, lvl_pos_embed, mask_flatten, spatial_shapes, level_start_index, valid_ratios = flattened  # src_flatten torch.Size([1, 1024, 256])
        # 4. 生成提示（Prompt）
        '''
        ...
        _, intermediate_preds = self.decoder(
            src_flatten, lvl_pos_embed, mask_flatten, spatial_shapes, 
