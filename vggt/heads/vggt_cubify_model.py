...
class EncoderProposals(Prompter):
    ...
    def train_single_image(self, output, image_size, topk):
        ...
        xyz_g = output.pred_xyz_g                              # [Nq,3] or [Nq,*]
        dims = output.pred_dims                                # [Nq,3]
        pose = output.pred_pose                                # [Nq,*]
        pose_g = output.pred_pose_g                            # [Nq,*]
        ...
        result.pred_boxes_3d_g = GeneralInstance3DBoxes(
            torch.cat((xyz_g, dims[:, [2, 1, 0]]), dim=-1),
            pose_g
        )
        result.object_desc = object_desc
        result.pred_proj_xy = proj_xy
    ...
    def train_inference(self, output, sensor, topk, intrinsic, extrinsic, gravity):
        results = []
        ...
        for index, output_ in enumerate(output):
            ...
            output_.pred_pose_g = torch.matmul(R[None, :, :], output_.pred_pose)
            output_.pred_xyz_g = output_xyz_world  #output_xyz
