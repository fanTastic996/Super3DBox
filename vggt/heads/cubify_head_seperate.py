...
class CubifyHead(nn.Module):
    ...
    def inference(
        ...
        for j in range(N_batch): # 一个个batch轮流来，因为cubify不支持
            ...
            fused_features = multiframe_fused_features
            # 4. 生成提示（Prompt）
            '''
            ...
            seq_result = []
            for k in range(fused_features.shape[0]): #each image
            
                prompts = self.prompting.get_image_prompts(
                    fused_features[k:k+1], mask_flatten[k:k+1], spatial_shapes, sensor
                )  #modules objects
                ...
                _, intermediate_preds = self.decoder(
                    fused_features[k:k+1], lvl_pos_embed, mask_flatten[k:k+1], spatial_shapes, 
                    level_start_index, valid_ratios[k:k+1], prompts, sensor
                )
                ...
                for prompter_index, (prompt, prompter) in enumerate(zip(prompts, prompters)):
                    ...
                    if prompt.has_output:
                        ...
                        prompt_start_idx += prompt.number_prompts
                        break  # 通常只有一个提示器产生输出
                seq_result.append(results[0].pred_boxes_3d.corners) #[0].pred_boxes_3d.corners) #[100,8,3]
    ...
    def forward(self, batched_inputs, aggregated_tokens_list, patch_start_idx, do_postprocess=True):
        """训练/推理的统一入口（实际调用推理函数）"""
        return self.inference(batched_inputs, aggregated_tokens_list, patch_start_idx)
