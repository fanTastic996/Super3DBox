...
class Trainer:
    ...
    def _load_resuming_checkpoint_mine(self, ckpt_path, cubify_path):
        ...
        # Load cubify Head : box_head
        cubify_checkpoint = torch.load(cubify_path, map_location="cpu")["model"]
        print(f"Loaded cubify model from {cubify_path}")
        # 过滤掉不需要的参数（通过 key 匹配）
        model_dict = self.model.box_head.state_dict()
        filtered_dict = OrderedDict()

        for k in cubify_checkpoint.keys():
            # 跳过包含 vggt_model 或 fusion_model 或 vggt_merger 的键
            if not (k.startswith("vggt_model") or k.startswith("fusion_model") or k.startswith("vggt_merger")) or k.startswith("frame_merger"):
    ...
    def _run_steps_on_batch_chunks(
        ...
        # print("chunked_batches",len(chunked_batches)) #2
        # exit(0)
        amp_type = self.optim_conf.amp.amp_dtype
        assert amp_type in ["bfloat16", "float16"], f"Invalid Amp type: {amp_type}"
        if amp_type == "bfloat16":
    ...
    def _apply_batch_repetition(self, batch: Mapping) -> Mapping:
        ...
        tensor_keys = [
            ...
            "cam_points", "world_points", "point_masks", "bbox_corners"
        ]         # TODO: changed by lyq
